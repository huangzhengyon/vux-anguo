        ...
        <div class="main">
          ...
          <div  v-transfer-dom>
            <popup class="jtempop" v-model="showholderForm" position="left" width="100%">
              ...
              <div class="holderMain" v-if="addholder">
                ...
                <div class="jtempopMain">
                  <div class="groupwrap" v-for="(item,key) in PolicyholderArgs" :class="{isRequired:item.isRequired}" :key="key">
                      <Group v-if="item.param == 'birth'">
                          <x-input title="年龄" type="number" @on-change="changeWritebirth" v-model="writeAge" placeholder="填写年龄"></x-input>
                      ...
                      <Group v-else>
                          ...
                          <div v-if="item.type=='int'">
                            <x-input :title="item.name" v-model="item.value" type="number" :placeholder="`请输入${item.name}`"></x-input>
                          </div>
                ...
                <box class="savebtn"  >
                    <x-button  @click.native="saveHolder('1')">保存</x-button>
                </box>
              </div>
              <div class="watchsmo" v-if="watchPeopleholder">
                <div class="position-horizontal-demo jtempopHeader">
                  <span @click="backtoHoldlist">返回</span>
                  <span class="spanTitle">查看详情</span>
                </div>
                <div class="personnalMain">
                  <group v-for="(item,key) in personnalDetail" :key="key" v-show="key!=='userId'&&key!=='applicantId'&&key!=='customerId'">
                    <!-- <x-input :title="key | toLocalName"  :value="item" :disabled=true text-align="right"></x-input> -->
                    <x-input :title="key | toLocalName" v-if="key == 'monthlyPayCode'" :value="item | tolocalType" :disabled=true text-align="right"></x-input>
                    <x-input :title="key | toLocalName" v-if="key == 'currentAssetsCode'" :value="item | tolocalType" :disabled=true text-align="right"></x-input>
                    <x-input :title="key | toLocalName" v-if="key == 'customerType'" :value="item=='1'?'投保人':'被保人'" :disabled=true text-align="right"></x-input>
                    <x-input :title="key | toLocalName" v-if="key == 'smokeHabit'" :value="item=='0'?'有':'无'" :disabled=true text-align="right"></x-input>
                    <x-input :title="key | toLocalName" v-if="key == 'isOnJob'" :value="item=='0'?'是':'否'" :disabled=true text-align="right"></x-input>
                    <x-input :title="key | toLocalName" v-if="key == 'isHKidentityCard'" :value="item=='0'?'是':'否'" :disabled=true text-align="right"></x-input>
                    <x-input :title="key | toLocalName" v-if="key == 'isSocialSecurity'" :value="item=='0'?'有':'无'" :disabled=true text-align="right"></x-input>
                    <x-input :title="key | toLocalName" v-if="key == 'certiType'" :value="item=='1'?'身份证':''" :disabled=true text-align="right"></x-input>
                    <x-input :title="key | toLocalName" v-if="key == 'maritalStatus'" :value="item | tolocalContent" :disabled=true text-align="right"></x-input>
                    <x-input :title="key | toLocalName" v-if="key == 'sex'" :value="item=='1'?'男':'女'" :disabled=true text-align="right"></x-input>
                    <x-input :title="key | toLocalName" v-if="key == 'explainInsured'" :value="item | tolocalRelation" :disabled=true text-align="right"></x-input>
                    <x-input :title="key | toLocalName" v-if="key!=='currentAssetsCode'&&key!=='monthlyPayCode'&&key!=='customerType'&&key!=='smokeHabit'&&key!=='isOnJob'&&key!=='isHKidentityCard'&&key!=='isSocialSecurity'&&key!=='certiType'&&key!=='maritalStatus'&&key!=='sex'&&key!=='explainInsured'" :value="item" :disabled=true text-align="right"></x-input>
                  </group>
                </div>
              </div>
            </popup>
            ...
            <div  v-transfer-dom>
              <popup class="jtempop" v-model="showInsuredForm" position="left" width="100%">
                ...
                <div class="holderMain" v-if="addInsured">
                  ...
                  <box class="savebtn"  >
                      <x-button  @click.native="saveHolder('2')">保存</x-button>
                  </box>
                </div>
                <div class="watchsmo" v-if="watchPeopleInsured">
                  <div class="position-horizontal-demo jtempopHeader">
                    <span @click="backtoInsuelist">返回</span>
                    <span class="spanTitle">查看详情</span>
                  </div>
                  <div class="personnalMain">
                    <group v-for="(item,key) in personnalDetail" :key="key" v-show="key!=='userId'&&key!=='applicantId'&&key!=='customerId'">
          ...
          <group class="fromgroup xgroup specail" >
            ...
            <div class="selectedBox" v-show="!isFading">
                <div class="InsuredBox smallsize" v-for="(item , key) in beneficiary" :key="key">
                  <cell class="from-cell from-cell-dl" is-link  :title="item.chineseName" :value="item.certiCode" @click.native="showMoreholder('3')"></cell>
                </div>
                <cell class="from-cell from-cell-dl emptyInfo smallsize" v-show="beneficiary == ''"  title="添加受益人" is-link @click.native="showMoreholder('3')"></cell>
            </div>
            ...
            <div  v-transfer-dom>
              <popup class="jtempop" v-model="showBeneficiaryForm" position="left" width="100%">
                ...
                <div class="holderMain" v-if="addBeneficiary">
                  ...
                  <box class="savebtn"  >
                      <x-button  @click.native="saveHolder('3')">保存</x-button>
                  </box>
                </div>
                <div class="watchsmo" v-if="watchBeneficiary">
                  <div class="position-horizontal-demo jtempopHeader">
                    <span @click="backtoBeneficiaryList">返回</span>
                    <span class="spanTitle">查看详情</span>
                  </div>
                  <div class="personnalMain">
                    <group v-for="(item,key) in personnalDetail" :key="key" v-show="key!=='userId'&&key!=='applicantId'&&key!=='customerId'">
                      <!-- <x-input :title="key | toLocalName"  :value="item" :disabled=true text-align="right"></x-input> -->
                      <x-input :title="key | toLocalName" v-if="key == 'monthlyPayType'" :value="item | tolocalType" :disabled=true text-align="right"></x-input>
                      <x-input :title="key | toLocalName" v-if="key == 'customerType'" :value="item=='1'?'投保人':'被保人'" :disabled=true text-align="right"></x-input>
                      <x-input :title="key | toLocalName" v-if="key == 'smokeHabit'" :value="item=='0'?'有':'无'" :disabled=true text-align="right"></x-input>
                      <x-input :title="key | toLocalName" v-if="key == 'isOnJob'" :value="item=='0'?'是':'否'" :disabled=true text-align="right"></x-input>
                      <x-input :title="key | toLocalName" v-if="key == 'isHKidentityCard'" :value="item=='0'?'是':'否'" :disabled=true text-align="right"></x-input>
                      ...
                      <x-input :title="key | toLocalName" v-if="key == 'explainInsured'" :value="item | tolocalRelation" :disabled=true text-align="right"></x-input>
                      <x-input :title="key | toLocalName" v-if="key == 'sex'" :value="item=='1'?'男':'女'" :disabled=true text-align="right"></x-input>
                      <x-input :title="key | toLocalName" v-if="key == 'benefitRatio'" :value="`${item}%`" :disabled=true text-align="right"></x-input>
                      <x-input :title="key | toLocalName" v-if="key!=='benefitRatio'&&key!=='monthlyPayType'&&key!=='customerType'&&key!=='smokeHabit'&&key!=='isOnJob'&&key!=='isHKidentityCard'&&key!=='isSocialSecurity'&&key!=='certiType'&&key!=='maritalStatus'&&key!=='sex'&&key!=='explainInsured'" :value="item" :disabled=true text-align="right"></x-input>
                    </group>
                  </div>
                </div>
              </popup>
          ...
          <!-- <div class="paymentMethod custom">
            <span>缴费方式</span>
            <div class="checkradio">
              <check-icon :value="payMethod" type="plain" @click.native="changePayMethod">年缴</check-icon>
              <check-icon :value="!payMethod" type="plain" @click.native="changePayMethod">月缴</check-icon>
            </div>
          </div> -->
...
export default {
  ...
  data () {
    return {
      ...
      checkedApplicantId:'',//获取的投保人ID
      checkedInsureId:'',//获取的投保人ID
      checkedBeneficiary:[],//获取的投保人ID
      premiumTax:'',//保费征税
      writeAge:'',//填写的年龄
      chooseAge:'',//选择的年龄
  ...
  filters:{
    ...
    toLocalName(str){
      let result 
      if(str == 'explainInsured'){
        result = "与投保人关系"
      }else if( str == 'chineseName'){
        result = '中文名'
      }else if(str == 'englishName'){
        result = '英文名'
      }else if( str == 'birth'){
        result = '出生日'
      }else if(str == 'nationality'){
        result = "国籍"
      }else if(str == 'placeAbode'){
        result = '居住地'
      }else if(str == 'sex'){
        result = '性别'
      }else if(str == 'maritalStatus'){
        result = '婚姻状态'
      }else if( str == 'certiType'){
        result = '证件类型'
      }else if(str == 'certiCode'){
        result = '证件号'
      }else if(str == 'isSocialSecurity'){
        result = '是否有社保'
      }else if(str == 'isHKidentityCard'){
        result = '是否持有香港证件'
      }else if(str == 'credentialsIndateYear'){
        result = '身份证有效期'
      }else if(str == 'identityAddress'){
        result = '身份证地址'
      }else if(str =='mobileNo'){
        result = '手机号码'
      }else if(str =='homeTelephone'){
        result = '家庭号码'
      }else if(str == 'degreeEducation'){
        result = '最高学历'
      }else if(str == 'mail'){
        result = '邮箱'
      }else if(str == 'isOnJob'){
        result = '是否在职'
      }else if(str == 'smokeHabit'){
        result = '吸烟习惯'
      }else if(str == 'cigarette'){
        result = '多少根/一天'
      }else if(str == 'smokeAge'){
        result = '烟龄'
      }else if(str == 'height'){
        result = '身高/cm'
      }else if(str == 'weight'){
        result = '体重/kg'
      }else if(str == 'companyName'){
        result = '公司名称'
      }else if(str == 'industry'){
        result = '所属行业'
      }else if(str == 'yearOfOffice'){
        result = '任职年期'
        ...
        result = '职位'
      }else if(str == 'jobContent'){
        result = '职位内容'
      }else if( str == 'monthlyPayType'){
        result = '月薪币种'
      }else if(str == 'monthlyPay'){
        result = '月薪'
      }else if(str == 'currentAssetsType'){
        result = '流动资产币种'
      }else if(str == 'currentAssets'){
        result = '流动资产'
      }else if(str == 'isOnJobDesc'){
        result = '在职描述'
      }else if(str == 'customerType'){
        result = '客户类型'
      }else if(str == 'benefitRatio'){
        result = '受益比例'
      }
      return result 
    }
  ...
  methods:{
    ...
    changeSelectbirth(val){//修改时间控件选择的年龄
      let vm = this 
    ...
    getCheckedCustomer(str){//获取已填写的客户
      let vm = this 
      var obj ;
      if(str == '1'){
        obj = {
    ...
    createPlan(){//修改预约填写基本信息
       let vm = this 
        ...
        if(vm.applicantId == ''){
          ...
          vm.$toast(`请填入${vm.payMin}和${vm.payMax}之间的数值`)
          return 
          ...
          if(!item.options){
            if(item.param == 'birth'){
              if(vm.chooseAge == ''){
                ...
                tObj[item.param] = vm.chooseAge.replace(/-/g,'')
              }
            }else if(item.param == 'certiCode'){
              if(Validator.isValid( item.value ) == false){
                tObj[item.param] = ''
                vm.$toast("身份证号码审核失败,请校验")
                return 
              }else{
                tObj[item.param] = item.value
              }
            }else{
              if(item.value == ''){
                vm.$toast('请填写完整信息,带*号的为必填选项')
                return false
              }else{
                tObj[item.param] = item.value
              }
