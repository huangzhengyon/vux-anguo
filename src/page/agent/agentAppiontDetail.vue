<template>
  <div class="agentAppointDetail">
      ...
      <div class="main">
          <Group>
            ...
            <Group v-if="isshowRemark=='1'">
              <datetime title="预约时间" format="YYYY-MM-DD HH:mm" :readonly="canselect" :start-date="nowDate" :end-date="endDate" v-model="appointmentTime" placeholder="请选择"></datetime>
            </Group>
            <cell title="预约时间" v-if="isshowRemark!=='1'" :value="appointmentTime"></cell>
            <x-textarea title="备注" class="beizhu" name="detail" v-show="detail.remark!==''" v-model="oldremark" readonly :show-counter="false"  ></x-textarea>
          </Group>
    ...
    <div class="bottom-x-button" v-show="!ischeck&&isshowRemark == '3'">
      <box class="submitBtn" v-if="checkPolicy" >
          <x-button class="default active"  @click.native="gotoWrite">填写保单</x-button>
      </box>
      <div class="haswrited" v-else>
...
export default {
  ...
  data () {
    return {
      ...
      emptyNext:'',//下一节点是否为空
      canselect:true,//时间控件是否可选
      writemore:false,//是否填写备注
      ischeck:true,//是否显示底部按钮
      checkPolicy:false,//是否完成填写保单
    }
  },
  mounted(){
    let vm = this 
  ...
  methods:{
    ...
    gotoUpload(){//补传资料
      let vm = this 
      vm.$router.push({path:'./amendAppiont',query:{orderNo:vm.orderNo}})
    },
    submitChange(){//提交处理状态
      let vm = this 
      if(vm.btnState == true){
        if(vm.isshowRemark == '1'){
          ...
          let obj = {
              servicePath:'KINGWORKS_004_002_004',
              jsonData: {
                  orderNo:vm.orderNo,
    ...
    getDetail(){//获取预约详情
      let vm =  this 
      let obj = {
            servicePath:'KINGWORKS_004_002_009',
            jsonData: {
                orderNo:vm.orderNo
            }
        }
          ...
          .then(function (response) {
            ...
            vm.$retCodeHandle2(data, function () {
              ...
              if(vm.detail.orderStatus == 140 || vm.detail.orderStatus == 160){//选择时间
                vm.isshowRemark = '1'
              }else if(vm.detail.orderStatus == 50 || vm.detail.orderStatus == 120 ){//补充资料
                vm.isshowRemark = '2'
